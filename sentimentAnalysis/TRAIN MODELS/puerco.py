...
class FocalLoss(nn.Module):
    def __init__(self, alpha=None, gamma=2, reduction='mean'):
        """
        Args:
            alpha: Puede ser None, un escalar, o una lista/tupla con un peso por clase.
                   Por ejemplo, alpha=[2.0, 1.0, 3.0] para 3 clases.
            gamma: Parámetro de enfoque en ejemplos difíciles (>= 0).
            reduction: 'mean' (promedio), 'sum' (suma) o 'none' (sin reducción).
        """
        super(FocalLoss, self).__init__()
        if alpha is not None:
            # Convierte alpha a tensor si es lista/tupla
    ...
    def forward(self, inputs, targets):
        ...
        Args:
            ...
            targets: Etiquetas verdaderas de tamaño [batch_size], con valores en [0, num_classes-1].
        """
        ce_loss = nn.CrossEntropyLoss(reduction='none')(inputs, targets)
        pt = torch.exp(-ce_loss)  # Probabilidad predicha para la clase verdadera

        # Si tenemos un vector alpha, escogemos el alpha correspondiente a la clase de cada ejemplo
...
class BetoMultiOutput(nn.Module):
    def __init__(
        ...
        # Ejemplo de MLP (puedes modificar el tamaño de la capa oculta):
        ...
        # Definir tus funciones de pérdida (Focal o CrossEntropy)  
        # Ejemplo de constructor con un vector alpha para FocalLoss
        if self.use_focal_loss:
            # Supongamos que alpha = [2.0, 1.0, 3.0] para 3 clases
            ...
            for c in range(num_candidates):
                if self.class_weights:
                    # asumiendo que `class_weights` es un dict con claves = nombre de columna y valor = tensor pesos
                    # sentiment_columns[c] -> 'Xóchitl' / 'Claudia' / ...
...
use_focal_loss = True  # Cambia a True para usar Focal Loss
...
model = BetoMultiOutput(
    ...
    num_labels_per_candidate=3, 
    num_candidates=4,
    alpha=[0.75,0.25,0.75],  # Solo si usas Focal Loss
    gamma=3   # Solo si usas Focal Loss
)
model.to(device)
