...
def cargar_comentarios():
    if os.path.exists('horaHora_diaEleccionN.csv'):
        comentarios = pd.read_csv('horaHora_diaEleccionN.csv',encoding='utf-8', quotechar='"', skipinitialspace=True)
    else:
        comentarios = pd.read_csv('horaHora_diaEleccion.csv',encoding='utf-8', quotechar='"', skipinitialspace=True)
...
def cargar_estado_actual():
    ...
    try:
        with open("estado_actual_he.json", "r", encoding='utf-8') as f:
...
def guardar_estado_actual():
    with open("estado_actual_he.json", "w", encoding='utf-8') as f:
...
def reconstruir_comentario_editado():
    # Definir los signos de puntuación permitidos
    allowed_punct = {'.', ',', ';', ':', '¿', '?', '¡', '!', '(', ')' }
    ...
    for token in comentario_original_palabras:
        # Si es una palabra (contiene caracteres alfanuméricos)
        if re.match(r'\b\w+\b', token):
            ...
            # Si no es palabra, es signo de puntuación u otro símbolo
            # Preserva el token solo si está en la lista permitida
            if token in allowed_punct:
                resultado.append(token)
    
    # Unir los tokens considerando espacios
    comentario_editado = ''
...
def procesar_palabra():
    ...
    try:
        ...
        comentarios_df.to_csv("horaHora_diaEleccionN.csv", index=False, quoting=csv.QUOTE_ALL, encoding='utf-8')
